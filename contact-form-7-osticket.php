...
Plugin URI: https://github.com/joecot/contact-form-7-osticket-integration
Description: Submit contact form 7 to OSTicket
Version: 1.0.1
Author: Joe Terranova
Author URI: https://www.joeterranova.net/
License: AGPLv3
Text Domain: contact-form-7-osticket-integration
*/
...
function cf7_osticket_before_send_mail($contact_form) {
	...
	if($parameters && count($parameters)) foreach($parameters as $key => $val) {
		if (!empty($key)) {
			$matches = Array();
			//replace placeholders in URL with form values
			if(strpos($val,'[') !== FALSE && preg_match_all('/\[([^]]+)]/',$val, $matches)){
				foreach($matches[1] as $match){
					if(isset($data[$match])) $val = str_replace('['.$match.']',$data[$match],$val);
				}
			}
			$post[$key] = $val;
		}
